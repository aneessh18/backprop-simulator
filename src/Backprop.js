...
function Backprop(props) {
  ...
  for(let epoch =0;epoch<epochs;epoch++)
  {
    ...
    for(let p=0;p<trainData.length;p++)
    {
      ...
      for(let i=1;i<l;i++)
      {
        ...
        for(let j=0;j<cur;j++)
        {
          if(previousUpdates[`b${nodeNo}`] === undefined)
          previousUpdates[`b${nodeNo}`] = 0;
          if(momentum !== 0)
          {
            let calc1 = `${momentum}*${previousUpdates[`b${nodeNo}`]}+(${1-momentum})*${deltas[nodeNo]}`;
            let currentUpdate = momentum*previousUpdates[`b${nodeNo}`]+(1-momentum)*deltas[nodeNo];
            console.log(currentUpdate);
            let calc2 = `${bias[nodeNo]} + ${learningRate}*${currentUpdate}`;
            previousUpdates[`b${nodeNo}`] = currentUpdate;
            bias[nodeNo] += learningRate*currentUpdate;
            weightUpdationOutputs.push([calc1, calc2, `b${nodeNo} ${bias[nodeNo]}`])
          }
          else
          {
          ...
          for(let k=0;k<prev;k++)
          {
            if(previousUpdates[`w${nodeNo}${k+1}`] === undefined)
            previousUpdates[`w${nodeNo}${k+1}`] = 0;
            if(momentum !== 0)
            {
              let calc1 = `${momentum}*${previousUpdates[`w${nodeNo}${k+1}`]}+(${1-momentum})*${deltas[nodeNo]}*${consumed[`${nodeNo}${k+1}`]}`;
              let currentUpdate = momentum*previousUpdates[`w${nodeNo}${k+1}`]+(1-momentum)*deltas[nodeNo]*consumed[`${nodeNo}${k+1}`];
              let calc2 = `${weights[`${nodeNo}${k+1}`]} + ${learningRate}*${currentUpdate}`;
              previousUpdates[`w${nodeNo}${k+1}`] = currentUpdate;
              weights[`${nodeNo}${k+1}`] += learningRate*currentUpdate;
              weightUpdationOutputs.push([calc1, calc2, `w${nodeNo}${k+1} ${weights[`${nodeNo}${k+1}`]}`])
            }
            else{
  ...
  for(let i=0;i<epochOutputs.length;i++)
  {
    ...
    for(let j=0;j<epochOutputs[i].length;j++)
    {
      ...
      if(j%3===2)
      {
        ...
        {
          // console.log(epochOutputs[i][j][k][2]);
          let index = (momentum ===0 ) ? 1:2;
          let z = epochOutputs[i][j][k][index].split(' ') ;
          let row = (momentum === 0) ?
          <tr>
            <td>{z[0]}</td>
            <td>{epochOutputs[i][j][k][0]}</td>
            <td>{z[1]}</td>
          </tr>
          :
          <tr>
  ...
  return (
    <div className="stochastic">
      ...
      <Input type="number" name={"Momentum"} value={momentum} setInput={setMomentum}/>
